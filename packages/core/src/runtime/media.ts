 ...
 * after `DOMContentLoaded` can lose the race against the resource-selection
 * task the parser already queued.
 */
...
function selectMediaTransitionSet(
  ...
  const compositionDuration = getCompositionDuration();
  for (const clip of clips) {
    if (
      (timeSeconds >= clip.start && timeSeconds < clip.end) ||
      (compositionDuration > 0 &&
        clip.el.tagName === "VIDEO" &&
        timeSeconds >= clip.end &&
        isClipVisibleAt(timeSeconds, clip.start, clip.end, compositionDuration))
    ) {
      inWindow.push(clip);
...
export function syncRuntimeMedia(params: {
  ...
  for (const clip of selectMediaTransitionSet(
    params.clips,
    params.timeSeconds,
    params.getCompositionDuration,
  )) {
    const { el } = clip;
